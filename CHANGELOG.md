...
### Changed
- Note frontmatter is now delimited with `+++` instead of `---`. `---` universally signals YAML frontmatter to other tools (Obsidian, Jekyll, Hugo, pandoc), which would try — and fail — to parse jd's TOML frontmatter; `+++` is the established convention for TOML frontmatter. Notes written by earlier jd versions with `---` keep working indefinitely — jd reads both delimiters — and are transparently migrated to `+++` the next time jd rewrites them (`tag`, `pin`, `property`, `rename`, `prepend`, etc.). No action needed; new notes just use `+++` going forward.
